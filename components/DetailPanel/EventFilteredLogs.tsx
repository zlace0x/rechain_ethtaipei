...
import { ChainId, getExplorer } from "../../lib/network";
      ...
      <ul className="text-xs nodrag">
        {filteredLogs &&
          filteredLogs.map((log) => (
            <EventListItem key={log.id} log={log} chainId={chainId} />
          ))}
      </ul>
    </div>
  );
...
export function EventListItem({ log, chainId }: { log: FormattedLog; chainId: ChainId }) {
  ...
  return (
    ...
    >
      <div className="flex flex-row gap-1">
        ...
        <a
          href={getExplorer(chainId, log.hash)}
